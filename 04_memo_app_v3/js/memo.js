...
function viewStorage(){
    ...
    function delLocalStorage() {
        ...
        function(e) {
            ...
            if(w_cnt >= 1){
                //const key = document.getElementById("textKey").value;//
                //const value = document.getElementById("textMemo").value;//
                let w_confirm = window.confirm("LocalStorageから選択されている" + w_cnt + "件を消除(delete)しますか?");
    ...
    function allClearlocalStorage(){
        ...
        allClear.addEventListener("click",
            function(e){
                e.preventDefault();
                let w_confirm = confirm("LocalStorageのデータをすべで削除(all clear)します。\nよろしいですか?");
                if(w_confirm === true){
                    localStorage.clear();
                    viewStorage();
                    let w_msg = "localStoragerのデータをすべて削除(all clear)しました。"
                    window.alert(w_msg);
                    document.getElementById("textKey").value = "";
                    document.getElementById("textMemo").value = "";  
                }
